...
class OVFormer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.clip_train = torch.load("datasets/metadata/lvis_train_clip_feature.pkl", map_location=self.device)
        self.clip_val = torch.load("datasets/metadata/lvis_val_clip_feature.pkl", map_location=self.device)
    ...
    def forward(self, batched_inputs):
        ...
        for i in range(len(batched_inputs)):
            if self.training:
                feature_clip = self.clip_train[batched_inputs[i]['image_id']].to(self.device)
            else:
                feature_clip = self.clip_val[batched_inputs[i]['image_id']].to(self.device).to(torch.float32)
