...
from ovformer.modeling.transformer_decoder.maskformer_transformer_decoder import TRANSFORMER_DECODER_REGISTRY
...
class VideoMultiScaleMaskedTransformerDecoder(nn.Module):
    ...
    def forward(self, x, mask_features, features_clip, mask = None):
        ...
        outputs_object, outputs_class, outputs_mask, attn_mask = self.forward_prediction_heads(output, mask_features, features_clip, attn_mask_target_size=size_list[0])
        ...
        for i in range(self.num_layers):
            ...
            outputs_object, outputs_class, outputs_mask, attn_mask = self.forward_prediction_heads(output, mask_features, features_clip, attn_mask_target_size=size_list[(i + 1) % self.num_feature_levels])
            predictions_object.append(outputs_object)
            predictions_class.append(outputs_class)
            predictions_mask.append(outputs_mask)
