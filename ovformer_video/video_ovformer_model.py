...
class VideoOVFormer(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        for i in range(len(batched_inputs)):
            ...
            for j in range(len(batched_inputs[i]['file_names'])):
                feature_clip = clip[batched_inputs[i]['file_names'][j]].to(self.device)
        ...
        if self.training:
            ...
            for i in range(math.ceil(num_frames / c)):
                ...
                mask_pred.append(outputs["pred_masks"][0])
            ...
            # mask_obj_result = outputs["pred_object_logits"][0]
            ...
            del mask_cls, mask_pred, mask_embed, out_cls, out_pred, out_embed
            ...
            return retry_if_cuda_oom(self.inference_video)(mask_cls_result, mask_obj_result, mask_pred_result, image_size, height, width)
    ...
    def inference_video(self, pred_cls, pred_obj, pred_masks, img_size, output_height, output_width):
        if len(pred_cls) > 0:
